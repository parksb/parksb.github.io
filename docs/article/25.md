...
* IMM(16bits): constant나 address가 담긴다. constant의 경우 $-2^{15}$부터 $2^{15} - 1$까지의 상수를 저장할 수 있다. address의 경우 rs의 base address에 offset으로 기능하거나 점프해야 하는 instruction까지의 거리를 저장한다.
