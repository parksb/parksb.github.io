...
웹의 복잡성이 폭발적으로 증가한 결정적인 시기를 하나만 고르자면, 웹이 초기의 문서 중심 시스템에서 벗어나 애플리케이션으로 발전하게 된 순간을 꼽을 수 있다. 2000년대 중반, 구글 맵은 사용자가 지도를 축소, 확대하거나, 드래그해서 지도의 새로운 영역을 로드하는 등 다양한 인터랙션을 선보였다. 이 모든 인터랙션에 화면을 전체 새로고침하는 동작은 필요하지 않았다. 본격적으로 Ajax(Asynchronous JavaScript and XML)를 이용해 웹 애플리케이션을 만들기 시작하면서[^jesse2005] 웹은 더 이상 클라이언트가 서버로부터 완성된 HTML 문서를 받아와 보여주기만 하는 시스템이 아니게 되었다.

![상단의 Server 타임라인에서 화살표가 우측으로 흐르고, 하단의 Client 타임라인도 우측으로 흐르는 시퀀스 다이어그램. 클라이언트가 서버로 2개의 비동기 요청을 보내고, 서버 응답을 받는 AJAX 통신 흐름을 표현함.](/images/45e51166-85cc-498e-bb82-93990d5d5116.svg)
_Ajax를 사용하면 자바스크립트로 비동기 HTTP 통신을 할 수 있다._

웹 애플리케이션이 보편화되면서 웹은 프론트엔드와 백엔드로 나뉘어 JSON API로 소통했다. 사용자가 접하는 대부분의 화면이 자바스크립트로 작성되었고, 많은 웹 퍼블리셔와 플래시 개발자들이 자바스크립트 개발자로의 전환을 압박받았다. 기업은 웹의 클라이언트 측을 개발하는 프로그래머를 '프론트엔드 개발자'라고 부르기 시작했다.
...
![SPA 렌더링 방식을 표현한 시퀀스 다이어그램. 클라이언트에서 첫 GET 요청에 대한 HTML 응답을 받고, JS 번들을 요청한 뒤 응답받아 렌더링이 끝나는 시점에 FCP와 TTI가 붉은 수직선으로 표시됨.](/images/845a3e26-73f2-4a8f-8490-0f77c7f206a1.svg)
_SPA는 비어있는 HTML 파일을 응답받고, 이후 화면 전환은 모두 자바스크립트로 처리한다._
...
문제를 극복하려는 방안으로 서버에서 HTML을 렌더링해 전달하는 SSR 방식이 다시 주목받았다. 이전의 SPA 렌더링 방식은 SSR과 구분해 CSR(Client-Side Rendering)이라고 불렀다. 서버에서 HTML을 제공한다는 점에서 PHP나 JSP로 만든 전통적인 웹을 떠올리기도 하지만, 현대적인 SSR은 SPA의 컴포넌트 기반 웹 프레임워크 생태계를 유지하면서 첫 렌더링만 서버에서 수행하고 이후 상태 관리와 상호작용은 클라이언트가 담당하는 방식으로 동작한다는 점에서 차이가 있다. 이때 클라이언트가 상태와 이벤트 핸들러, 프레임워크 내부 로직 등이 포함된 자바스크립트를 로드해 인터랙션을 활성화하는 과정을 하이드레이션(Hydration)이라고 한다.

![SSR 방식을 표현한 시퀀스 다이어그램. FCP는 첫 GET 요청에 대한 HTML 응답을 받는 시점에, TTI는 하이드레이션이 끝나는 시점에 붉은 수직선으로 표시됨.](/images/c1af3b2f-1ed9-4799-8c7b-6dd6dd35e623.svg)
_SSR 방식은 첫 요청에서 사전 렌더링된 HTML을 응답받아 FCP 지표를 개선한다._

이러한 방식의 SSR의 병목은 크게 세 곳이다. 첫째, 서버는 모든 데이터가 준비돼야 렌더링을 하고 클라이언트에게 응답할 수 있다. 둘째, 클라이언트는 하이드레이션을 위해 서버로부터 모든 HTML과 자바스크립트 파일을 받아야 한다. 셋째, 클라이언트에서 하이드레이션이 완료되기 전까지는 사용자가 아무런 상호작용을 할 수 없다.[^dan2021] 세 개의 병목은 차례대로 이어져 있어서 이전 단계가 완료되기 전까지는 다음 단계를 진행할 수 없다. 이러한 병목을 완화하는 방법으로 점진적 렌더링(Progressive rendering)이 도입되었다. 점진적 렌더링은 전체 콘텐츠를 한 번에 완성해 응답하는 대신, 준비된 부분부터 우선 전송하는 렌더링 방식이다. 단일 요청-응답 사이클을 스트리밍할 수 있는 청크 단위로 쪼갬으로써 병목을 완화한 셈이다. 이를 통해 사용자는 화면의 일부라도 더 빠르게 볼 수 있다.

기존에 클라이언트에서 처리하던 영역을 서버로 이전하는 시도는 계속되고 있다. 리액트 서버 컴포넌트(RSC, React Server Component)는 서버에서 실행되는 리액트 컴포넌트다. 서버 컴포넌트는 파일시스템, 데이터베이스와 같은 서버 자원에 직접 접근할 수 있다. 서버는 컴포넌트 트리 생성해 직렬화된 형식으로 클라이언트에 스트리밍하며, 클라이언트는 이를 재조립해 최종 UI를 구성한다. 이때 서버 컴포넌트는 클라이언트 번들에 포함되지 않기 때문에 하이드레이션이 불필요하다. 한편, 개발자가 어떤 컴포넌트가 서버에서만 실행되고, 어떤 컴포넌트가 클라이언트에 제공되는지 인지해야 하므로 오히려 복잡도가 높아졌다는 비판도 있다. RSC에 대한 의견은 여전히 엇갈리지만, 서버 중심 접근을 통해 클라이언트의 오버헤드를 최소화하려는 생태계의 큰 흐름은 뚜렷하다. 실제로 클라이언트에서 실행되는 자바스크립트의 양을 줄임으로써 얻는 성능 개선도 분명하다. 많은 프레임워크들이 유사한 방향의 문제의식을 바탕으로 SSR 방식을 설계하고 있다. 어떻게 보면 클라이언트-서버 구조에서 비롯되는 웹의 자정작용일지도 모르겠다.
...
자기서술적 메시지 제약은 필딩이 함께 제시한 HATEOAS(Hypermedia as the Engine of Application State, 애플리케이션 상태 엔진으로서의 하이퍼미디어) 제약과도 밀접히 연결된다. HATEOAS는 서로 연결된 웹 페이지들의 네트워크를 하나의 상태 기계로 보고, 사용자가 하이퍼링크를 통해 다음 상태로 전이하는 방식으로 애플리케이션이 동작해야 한다는 제약이다. 앞서 살펴본 예시의 작동 방식 그 자체이며, 이 제약을 따르면 사용자가 인간이 아닌 기계인 경우에도 애플리케이션의 상태를 문제없이 탐색할 수 있다. HATEOAS는 'Representational State Transfer'라는 용어가 직접적으로 가리키는 의미이기도 하다.

![하이퍼링크를 기반으로 상태 전이를 수행하는 모습을 표현한 상태 다이어그램.](/images/3dbd611c-98b6-4076-9acd-bc68004d8a2d.svg)
_하이퍼미디어는 상태 기계다._
